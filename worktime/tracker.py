        ...
        for output in outputs:
            if output.get("focused", False):
                ...
                for monitor in sct.monitors[1:]:  # skip index 0
                    if (
                        monitor["left"] == output["rect"]["x"]
                        and monitor["top"] == output["rect"]["y"]
                        and monitor["width"] == output["rect"]["width"]
                        and monitor["height"] == output["rect"]["height"]
                    ):
                        logging.debug(
                            "Found focused monitor via swaymsg: {}".format(monitor)
                        )
                        return monitor
...
def take_screenshot_grim(monitor_index=None):
    ...
    if monitor_index is not None:
        ...
        if outputs and 0 <= monitor_index < len(outputs):
            ...
            logging.debug(
                "Monitor index {} out of range, using active monitor".format(
                    monitor_index
                )
            )
    ...
    if monitor.get_dbus_idle() < args.interval * 1000:
        ...
        try:
            ...
            try:
                ...
                grim_monitor = (
                    args.monitor - 1 if args.monitor and args.monitor > 0 else None
                )
                img = take_screenshot_grim(grim_monitor)
                logging.debug("Screenshot taken with grim")
            except Exception as grim_error:
                logging.error(
                    "Both mss and grim failed: mss={}, grim={}".format(e, grim_error)
                )
                return
...
def main(args):
    ...
    parser.add_argument(
        "--interval", "-i", help="interval time [seconds]", type=int, default=5 * 60
    )
    parser.add_argument(
        "--keep", "-k", help="number of days to keep", type=int, default=21
    )
